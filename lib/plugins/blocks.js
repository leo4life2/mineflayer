const { Vec3 } = require('vec3')
const assert = require('assert')
const Painting = require('../painting')
const { onceWithCleanup } = require('../promise_utils')

const { OctahedronIterator } = require('prismarine-world').iterators

module.exports = inject

const paintingFaceToVec = [
  new Vec3(0, 0, -1),
  new Vec3(-1, 0, 0),
  new Vec3(0, 0, 1),
  new Vec3(1, 0, 0)
]

const dimensionNames = {
  '-1': 'minecraft:nether',
  0: 'minecraft:overworld',
  1: 'minecraft:end'
}

function inject (bot, { version, storageBuilder, hideErrors }) {
  const Block = require('prismarine-block')(bot.registry)
  const Chunk = require('prismarine-chunk')(bot.registry)
  const World = require('prismarine-world')(bot.registry)
  const paintingsByPos = {}
  const paintingsById = {}

  // Block-state remapper (remap-on-read). On servers whose global block-state
  // table diverges from vanilla (registry-inflating mods), raw palette ids
  // decode to the wrong vanilla block. A calibration service may install a
  // remapper here; world storage stays raw and only reads translate.
  // Contract (duck-typed, see src/utils/blockStateCalibration.js):
  //   blockFromChunk(chunk, posInChunk) -> Block | null (null = raw is correct)
  //   blockFromStateId(rawStateId)      -> Block        (always returns a block)
  //   remapBlock(rawDecodedBlock)       -> Block | null (null = raw is correct)
  //   mapStateId(rawStateId)            -> number | null
  bot._blockStateRemapper = null
  bot.setBlockStateRemapper = (remapper) => {
    bot._blockStateRemapper = remapper || null
    // World may not exist yet (created on login); switchWorld installs then.
    if (bot.world && typeof bot.world.setStateRemapper === 'function') {
      bot.world.setStateRemapper(bot._blockStateRemapper)
    }
  }

  function addPainting (painting) {
    paintingsById[painting.id] = painting
    paintingsByPos[painting.position] = painting
  }

  function deletePainting (painting) {
    delete paintingsById[painting.id]
    delete paintingsByPos[painting.position]
  }

  function delColumn (chunkX, chunkZ) {
    bot.world.unloadColumn(chunkX, chunkZ)
  }

  function addColumn (args) {
    if (!args.bitMap && args.groundUp) {
      // stop storing the chunk column
      delColumn(args.x, args.z)
      return
    }
    let column = bot.world.getColumn(args.x, args.z)
    if (!column) {
      // Allocates new chunk object while taking world's custom min/max height into account
      column = new Chunk({ minY: bot.game.minY, worldHeight: bot.game.height })
    }

    try {
      column.load(args.data, args.bitMap, args.skyLightSent, args.groundUp)
      if (args.biomes !== undefined) {
        column.loadBiomes(args.biomes)
      }
      if (args.skyLight !== undefined) {
        column.loadParsedLight(args.skyLight, args.blockLight, args.skyLightMask, args.blockLightMask, args.emptySkyLightMask, args.emptyBlockLightMask)
      }
      bot.world.setColumn(args.x, args.z, column)
    } catch (e) {
      bot.emit('error', e)
    }
  }

  async function waitForChunksToLoad () {
    const dist = 2
    // This makes sure that the bot's real position has been already sent
    if (!bot.entity.height) await onceWithCleanup(bot, 'chunkColumnLoad', { timeout: 10000 })
    const pos = bot.entity.position
    const center = new Vec3(pos.x >> 4 << 4, 0, pos.z >> 4 << 4)
    // get corner coords of 5x5 chunks around us
    const chunkPosToCheck = new Set()
    for (let x = -dist; x <= dist; x++) {
      for (let y = -dist; y <= dist; y++) {
        // ignore any chunks which are already loaded
        const pos = center.plus(new Vec3(x, 0, y).scaled(16))
        if (!bot.world.getColumnAt(pos)) chunkPosToCheck.add(pos.toString())
      }
    }

    if (chunkPosToCheck.size) {
      return new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
          bot.world.off('chunkColumnLoad', waitForLoadEvents)
          reject(new Error(`Timeout waiting for ${chunkPosToCheck.size} chunks to load after 10000ms`))
        }, 10000)

        function waitForLoadEvents (columnCorner) {
          chunkPosToCheck.delete(columnCorner.toString())
          if (chunkPosToCheck.size === 0) { // no chunks left to find
            clearTimeout(timeout)
            bot.world.off('chunkColumnLoad', waitForLoadEvents) // remove this listener instance
            resolve()
          }
        }

        // begin listening for remaining chunks to load
        bot.world.on('chunkColumnLoad', waitForLoadEvents)
      })
    }
  }

  function getMatchingFunction (matching) {
    if (typeof (matching) !== 'function') {
      if (!Array.isArray(matching)) {
        matching = [matching]
      }
      return isMatchingType
    }
    return matching

    function isMatchingType (block) {
      return block === null ? false : matching.indexOf(block.type) >= 0
    }
  }

  function isBlockInSection (section, matcher) {
    if (!section) return false // section is empty, skip it (yay!)
    // If the chunk use a palette we can speed up the search by first
    // checking the palette which usually contains less than 20 ids
    // vs checking the 4096 block of the section. If we don't have a
    // match in the palette, we can skip this section.
    if (section.palette) {
      // Palette entries are RAW server state ids; on a state-table-diverged
      // server they must be remapped before decoding, or the palette
      // pre-filter matches (and misses) the wrong vanilla blocks.
      const remapper = bot._blockStateRemapper
      for (const stateId of section.palette) {
        const block = remapper ? remapper.blockFromStateId(stateId) : Block.fromStateId(stateId, 0)
        if (matcher(block)) {
          return true // the block is in the palette
        }
      }
      return false // skip
    }
    return true // global palette, the block might be in there
  }

  function getFullMatchingFunction (matcher, useExtraInfo) {
    if (typeof (useExtraInfo) === 'boolean') {
      return fullSearchMatcher
    }

    return nonFullSearchMatcher

    function nonFullSearchMatcher (point) {
      const block = blockAt(point, true)
      return matcher(block) && useExtraInfo(block)
    }

    function fullSearchMatcher (point) {
      return matcher(bot.blockAt(point, useExtraInfo))
    }
  }

  // F65 block-scan prefilter: for plain type-id matchers the swept cell's
  // STATE ID alone decides the match, so the full Block construction
  // (fullSearchMatcher -> blockAt per cell, the dominant stall stack of the
  // perf baseline) is only paid for cells that can actually match.
  // Skip-safety, remap included: bot.world.getBlockStateId returns the
  // REMAPPED id where the remapper is confident, whose decoded type is
  // exactly what blockAt's block.type would be; ambiguous states keep their
  // raw id (their true block is a type-undefined synthetic that can never
  // id-match — a raw-type false PASS just falls through to the exact
  // matcher); modded-range ids decode to no registry entry and are skipped
  // (their true block is likewise type-undefined). A skipped cell is
  // therefore always a cell the exact matcher would reject.
  function getStateIdPrefilter (matching) {
    if (typeof matching === 'function') return null
    const ids = new Set(Array.isArray(matching) ? matching : [matching])
    const byStateId = bot.registry.blocksByStateId
    return (cursor) => {
      const meta = byStateId[bot.world.getBlockStateId(cursor)]
      return meta !== undefined && ids.has(meta.id)
    }
  }

  bot.findBlocks = (options) => {
    const matcher = getMatchingFunction(options.matching)
    const point = (options.point || bot.entity.position).floored()
    const maxDistance = options.maxDistance || 16
    const count = options.count || 1
    const useExtraInfo = options.useExtraInfo || false
    const fullMatcher = getFullMatchingFunction(matcher, useExtraInfo)
    const prefilter = getStateIdPrefilter(options.matching)
    const start = new Vec3(Math.floor(point.x / 16), Math.floor(point.y / 16), Math.floor(point.z / 16))
    const it = new OctahedronIterator(start, Math.ceil((maxDistance + 8) / 16))
    // the octahedron iterator can sometime go through the same section again
    // we use a set to keep track of visited sections
    const visitedSections = new Set()

    let blocks = []
    let startedLayer = 0
    let next = start
    while (next) {
      const column = bot.world.getColumn(next.x, next.z)
      const sectionY = next.y + Math.abs(bot.game.minY >> 4)
      const totalSections = bot.game.height >> 4
      if (sectionY >= 0 && sectionY < totalSections && column && !visitedSections.has(next.toString())) {
        const section = column.sections[sectionY]
        if (useExtraInfo === true || isBlockInSection(section, matcher)) {
          const begin = new Vec3(next.x * 16, sectionY * 16 + bot.game.minY, next.z * 16)
          const cursor = begin.clone()
          const end = cursor.offset(16, 16, 16)
          for (cursor.x = begin.x; cursor.x < end.x; cursor.x++) {
            for (cursor.y = begin.y; cursor.y < end.y; cursor.y++) {
              for (cursor.z = begin.z; cursor.z < end.z; cursor.z++) {
                if (prefilter && !prefilter(cursor)) continue
                if (fullMatcher(cursor) && cursor.distanceTo(point) <= maxDistance) blocks.push(cursor.clone())
              }
            }
          }
        }
        visitedSections.add(next.toString())
      }
      // If we started a layer, we have to finish it otherwise we might miss closer blocks
      if (startedLayer !== it.apothem && blocks.length >= count) {
        break
      }
      startedLayer = it.apothem
      next = it.next()
    }
    blocks.sort((a, b) => {
      return a.distanceTo(point) - b.distanceTo(point)
    })
    // We found more blocks than needed, shorten the array to not confuse people
    if (blocks.length > count) {
      blocks = blocks.slice(0, count)
    }
    return blocks
  }

  function findBlock (options) {
    const blocks = bot.findBlocks(options)
    if (blocks.length === 0) return null
    return bot.blockAt(blocks[0])
  }

  function blockAt (absolutePoint, extraInfos = true) {
    let block = bot.world.getBlock(absolutePoint)
    // null block means chunk not loaded
    if (!block) return null

    // Fallback remap for world implementations without the remap-on-read
    // hook (setStateRemapper): translate the raw-decoded block here so
    // bot.blockAt stays correct even on an unpatched prismarine-world.
    if (bot._blockStateRemapper && typeof bot.world.setStateRemapper !== 'function') {
      const mapped = bot._blockStateRemapper.remapBlock(block)
      if (mapped) block = mapped
    }

    if (extraInfos) {
      block.painting = paintingsByPos[block.position]
    }

    return block
  }

  // ===== Visibility helpers ported from src/agent/library/world.js =====
  function normalizeShapes (shapes) {
    if (!Array.isArray(shapes)) return []
    const flattened = []
    const stack = [...shapes]
    while (stack.length > 0) {
      const entry = stack.pop()
      if (!Array.isArray(entry)) continue
      if (entry.length === 6 && entry.every(value => typeof value === 'number' && Number.isFinite(value))) {
        flattened.push(entry)
      } else {
        for (const sub of entry) stack.push(sub)
      }
    }
    return flattened
  }

  function computeFilledVolume (shapes) {
    const boxes = normalizeShapes(shapes)
    let volumeSum = 0
    for (const box of boxes) {
      const [xmin, ymin, zmin, xmax, ymax, zmax] = box
      const dx = Math.max(0, Math.min(1, xmax) - Math.max(0, xmin))
      const dy = Math.max(0, Math.min(1, ymax) - Math.max(0, ymin))
      const dz = Math.max(0, Math.min(1, zmax) - Math.max(0, zmin))
      volumeSum += dx * dy * dz
    }
    return Math.max(0, Math.min(1, volumeSum))
  }

  function isFullCubeShape (shapes) {
    const boxes = normalizeShapes(shapes)
    if (boxes.length !== 1) return false
    const b = boxes[0]
    return b[0] === 0 && b[1] === 0 && b[2] === 0 && b[3] === 1 && b[4] === 1 && b[5] === 1
  }

  // Registry-backed light-opacity lookup. prismarine-block instances do not
  // carry filterLight, so consult the version registry by block id; blocks
  // unknown to the registry (e.g. runtime-registered modded blocks) return
  // null and fall through to the geometry heuristics below.
  const filterLightCache = new Map()
  function blockFilterLight (block) {
    if (!block || typeof block.type !== 'number') return null
    if (filterLightCache.has(block.type)) return filterLightCache.get(block.type)
    let value = null
    try {
      const data = bot.registry.blocks[block.type]
      if (data && typeof data.filterLight === 'number') value = data.filterLight
    } catch (_) {}
    filterLightCache.set(block.type, value)
    return value
  }

  function isSeeThroughBlock (block) {
    if (!block) return true
    if (block.boundingBox === 'empty') return true
    if (!block.shapes || block.shapes.length === 0) return true

    // Vision-occlusion is decided by LIGHT ATTENUATION, not render
    // transparency. minecraft-data's `transparent` is a client rendering
    // flag: it is true for leaves (which made the bot watch mobs through
    // solid canopy) and glowstone alike. `filterLight` is the game's own
    // light-opacity: 0 for glass / stained glass / panes / barrier
    // (visually clear), >= 1 for leaves / ice / slime / spawner
    // (light-attenuating). A block the game says attenuates light occludes
    // vision; a render-transparent block that passes light untouched is
    // clear glass. Liquids (water/lava) never reach this rule: their
    // bounding box is 'empty', handled above.
    const filterLight = blockFilterLight(block)
    if (block.transparent === true && filterLight === 0) return true

    const name = (block.name || '').toLowerCase()

    // Last-resort name fallback for the one vanilla block whose light data
    // cannot express its looks: tinted glass is DESIGNED to block light
    // (filterLight 15) while remaining visually clear.
    if (name.includes('tinted_glass')) return true

    try {
      if (typeof block.getProperties === 'function') {
        const props = block.getProperties()
        if (props && props.open === true) return true
      }
    } catch (_) {}

    // Geometry verdict BEFORE the name hints: a full-cube collision shape is
    // an occluder no matter what its name contains. (Previously the hint
    // scan ran first, so grass_block leaked via the 'grass' hint and
    // flowering_azalea_leaves via 'flower' — solid ground that did not
    // occlude.)
    if (isFullCubeShape(block.shapes)) return false

    const porousHints = [
      'fence', 'fence_gate', 'pane', 'bars', 'ladder', 'scaffolding', 'rail', 'chain', 'sign', 'banner', 'torch',
      'button', 'lever', 'tripwire', 'pressure_plate', 'carpet', 'candle', 'end_rod', 'pointed_dripstone', 'vine',
      'bamboo', 'sapling', 'flower', 'grass'
    ]
    for (const hint of porousHints) {
      if (name.includes(hint)) return true
    }

    const filled = computeFilledVolume(block.shapes)
    if (filled <= 0.6) return true

    return false
  }

  // if passed in block is within line of sight to the bot, returns true
  // also works on anything with a position value
  // F65: the sample-point set is a pure function of the block's shape set,
  // which is fully determined by its state id — memoized per state so the
  // per-candidate normalize+sample rebuild (its own stall class once the
  // block-read storm shrank) runs once per state, not once per call.
  // Callers iterate the returned array read-only.
  // Calibration-minted blocks (block.calibration set) are excluded: their
  // shapes can hot-swap in place when a deferred jar derivation lands, and
  // registry-static state ids are the only safe memo keys.
  const samplePointsByState = new Map()
  function enumerateBlockSamplePoints (block) {
    const memoKey = (typeof block.stateId === 'number' && block.calibration === undefined) ? block.stateId : null
    if (memoKey !== null) {
      const cached = samplePointsByState.get(memoKey)
      if (cached !== undefined) return cached
    }
    const samples = computeBlockSamplePoints(block)
    if (memoKey !== null) {
      if (samplePointsByState.size >= 65536) samplePointsByState.clear()
      samplePointsByState.set(memoKey, samples)
    }
    return samples
  }

  function computeBlockSamplePoints (block) {
    if (block.boundingBox === 'empty' || !block.shapes || block.shapes.length === 0) {
      return [{ x: 0.5, y: 0.5, z: 0.5 }]
    }

    const shapes = normalizeShapes(block.shapes)
    if (shapes.length === 0) {
      return [{ x: 0.5, y: 0.5, z: 0.5 }]
    }

    const samples = []
    const insetAmount = 0.02

    for (const box of shapes) {
      const [xmin, ymin, zmin, xmax, ymax, zmax] = box
      const xs = [xmin, xmax]
      const ys = [ymin, ymax]
      const zs = [zmin, zmax]
      const center = {
        x: (xmin + xmax) / 2,
        y: (ymin + ymax) / 2,
        z: (zmin + zmax) / 2
      }
      const pushSample = (sx, sy, sz) => {
        const inset = (value, centerValue) => centerValue + (value - centerValue) * (1 - insetAmount)
        samples.push({
          x: inset(sx, center.x),
          y: inset(sy, center.y),
          z: inset(sz, center.z)
        })
      }

      for (const x of xs) {
        for (const y of ys) {
          for (const z of zs) {
            pushSample(x, y, z)
          }
        }
      }

      // Add center of each face for more coverage
      const faceCenters = [
        { x: xmin, y: center.y, z: center.z },
        { x: xmax, y: center.y, z: center.z },
        { x: center.x, y: ymin, z: center.z },
        { x: center.x, y: ymax, z: center.z },
        { x: center.x, y: center.y, z: zmin },
        { x: center.x, y: center.y, z: zmax }
      ]
      for (const center of faceCenters) {
        pushSample(center.x, center.y, center.z)
      }
    }

    const unique = new Map()
    for (const sample of samples) {
      const key = `${sample.x.toFixed(4)},${sample.y.toFixed(4)},${sample.z.toFixed(4)}`
      if (!unique.has(key)) unique.set(key, sample)
    }
    return Array.from(unique.values())
  }

  function canSeeBlock (block) {
    if (!block || !block.position) {
      return false
    }

    // Shapeless targets (empty bounding box or no collision shapes — crops,
    // torches, rails, liquids) can never be HIT by the sample raycast below:
    // the matcher requires iter.intersect() on the target's shapes, and an
    // empty shape intersects nothing, so such blocks were permanently
    // "invisible". Visibility for them is instead defined as "an
    // occluder-only ray REACHES the target cell" — the wall-sign epsilon
    // fallback generalized. Crops in the open stay visible (the ray reaches
    // them); buried torches/liquids behind rock are honestly occluded.
    if (block.boundingBox === 'empty' || !block.shapes || normalizeShapes(block.shapes).length === 0) {
      return canSeeCell(block)
    }

    const samples = enumerateBlockSamplePoints(block)
    if (samples.length === 0) {
      const targetCenterFallback = block.position.offset(0.5, 0.5, 0.5)
      return canSeePoint(targetCenterFallback)
    }

    const headPos = bot.entity.position.offset(0, bot.entity.eyeHeight, 0)

    const match = (inputBlock, iter, targetPos) => {
      if (!inputBlock) return false
      const intersects = iter.intersect(inputBlock.shapes, inputBlock.position)
      if (!intersects) return false
      if (inputBlock.position.equals(block.position)) {
        return inputBlock.position.offset(0.5, 0.5, 0.5).distanceTo(targetPos) < 1.0
      }
      return !isSeeThroughBlock(inputBlock)
    }

    const isWallSign = block.name && block.name.includes('_wall_sign')

    for (const sample of samples) {
      const baseTargetPos = block.position.offset(sample.x, sample.y, sample.z)
      let targetPos = baseTargetPos

      if (isWallSign) {
        const toHead = headPos.minus(baseTargetPos)
        const dist = toHead.norm()
        if (dist > 1e-4) {
          const epsilon = Math.min(0.2, Math.max(0, dist - 0.02))
          if (epsilon > 0) {
            const offsetVec = toHead.scaled(epsilon / dist)
            targetPos = baseTargetPos.plus(offsetVec)
          }
        }
      }

      const dir = targetPos.minus(headPos)
      const range = dir.norm() + 0.75

      try {
        const rayMatch = (inputBlock, iterator) => match(inputBlock, iterator, targetPos)
        // Contract with worldsync.raycast (F65): match() intersects the
        // block's shapes before anything else, so a zero-shape block can
        // never match — declaring it lets the ray skip constructing air.
        rayMatch.shapelessNever = true
        const hitBlock = bot.world.raycast(headPos, dir.normalize(), range, rayMatch)
        const visible = !!(hitBlock && hitBlock.position && hitBlock.position.equals(block.position))
        if (visible) return true
        if (block.name && block.name.includes('sign')) {
          if (!hitBlock) {
            const fallbackVisible = canSeePoint(targetPos)
            console.log(`[canSeeBlock] Sign sample (${sample.x.toFixed(2)},${sample.y.toFixed(2)},${sample.z.toFixed(2)}) fallbackVisible=${fallbackVisible}`)
            if (fallbackVisible) return true
          } else {
            const blockerPos = hitBlock.position ? `(${hitBlock.position.x},${hitBlock.position.y},${hitBlock.position.z})` : '(unknown)'
            const signPos = `(${block.position.x},${block.position.y},${block.position.z})`
            const headStr = `(${headPos.x.toFixed(2)},${headPos.y.toFixed(2)},${headPos.z.toFixed(2)})`
            const targetStr = `(${targetPos.x.toFixed(2)},${targetPos.y.toFixed(2)},${targetPos.z.toFixed(2)})`
            const blockerCenter = hitBlock.position ? hitBlock.position.offset(0.5, 0.5, 0.5) : null
            const blockerDist = blockerCenter ? blockerCenter.distanceTo(headPos).toFixed(3) : 'n/a'
            const targetDist = targetPos.distanceTo(headPos).toFixed(3)
            const entryDist = (() => {
              if (!hitBlock.shapes || hitBlock.shapes.length === 0) return 'n/a'
              try {
                const unionBoxes = normalizeShapes(hitBlock.shapes)
                if (unionBoxes.length === 0) return 'n/a'
                let minEntry = Number.POSITIVE_INFINITY
                const rayDir = dir.normalize()
                for (const box of unionBoxes) {
                  const iterator = new bot.registry.iterators.RaycastIterator(headPos, rayDir, range)
                  const entry = iterator.intersect([box], hitBlock.position)
                  if (entry) {
                    const dist = entry.pos.distanceTo(headPos)
                    if (dist < minEntry) minEntry = dist
                  }
                }
                return isFinite(minEntry) ? minEntry.toFixed(3) : 'n/a'
              } catch (err) {
                return 'n/a'
              }
            })()
            console.log(`[canSeeBlock] Sign blocked sample (${sample.x.toFixed(2)},${sample.y.toFixed(2)},${sample.z.toFixed(2)}) blocker=${hitBlock.name} blockerPos=${blockerPos} blockerEntry=${entryDist} blockerDist=${blockerDist} targetDist=${targetDist} signPos=${signPos} head=${headStr} target=${targetStr}`)
          }
        }
      } catch (err) {
        continue
      }
    }

    return false
  }

  // Check line of sight to an arbitrary point in space.
  // Returns true if no non-see-through block intersects the ray to the point.
  function canSeePoint (targetPos) {
    if (!targetPos) return false
    const headPos = bot.entity.position.offset(0, bot.entity.eyeHeight, 0)
    const dir = targetPos.minus(headPos)
    const range = dir.norm() + 0.75

    const match = (inputBlock, iter) => {
      if (!inputBlock) return false
      const intersects = iter.intersect(inputBlock.shapes, inputBlock.position)
      if (!intersects) return false
      return !isSeeThroughBlock(inputBlock)
    }
    // F65 raycast contract: shape intersect gates the match, so zero-shape
    // blocks can never match — the ray may skip constructing them.
    match.shapelessNever = true

    let hitBlock = null
    try {
      hitBlock = bot.world.raycast(headPos, dir.normalize(), range, match)
    } catch (_) {
      return false
    }
    return hitBlock === null
  }

  // Occluder-only line of sight to a block CELL, for targets the sample
  // raycast cannot hit (no collision shapes). Three deliberate differences
  // from a plain canSeePoint(cell center):
  // - the ray is epsilon-shortened toward the head (the wall-sign nudge) so
  //   a target point sitting on a neighbor's face plane is not swallowed by
  //   that neighbor's shape;
  // - the ray TERMINATES at the (shortened) target instead of overshooting
  //   +0.75 past it, so the support block behind/below the target (a crop's
  //   farmland when looked at from above) can never occlude its own crop;
  // - the target's own cell is exempt from occlusion.
  function canSeeCell (targetBlock) {
    if (!targetBlock || !targetBlock.position) return false
    const headPos = bot.entity.position.offset(0, bot.entity.eyeHeight, 0)
    const baseTargetPos = targetBlock.position.offset(0.5, 0.5, 0.5)
    const toHead = headPos.minus(baseTargetPos)
    const dist = toHead.norm()
    if (dist <= 1e-4) return true // head inside the target cell

    const epsilon = Math.min(0.2, Math.max(0, dist - 0.02))
    const targetPos = epsilon > 0 ? baseTargetPos.plus(toHead.scaled(epsilon / dist)) : baseTargetPos
    const dir = targetPos.minus(headPos)
    const range = dir.norm()
    if (range <= 1e-4) return true

    const match = (inputBlock, iter) => {
      if (!inputBlock || !inputBlock.position) return false
      if (inputBlock.position.equals(targetBlock.position)) return false // target cell exempt
      if (!iter.intersect(inputBlock.shapes, inputBlock.position)) return false
      return !isSeeThroughBlock(inputBlock)
    }
    // F65 raycast contract: every return path for a zero-shape block is
    // false (the target-cell exemption also returns false, so a skipped
    // zero-shape block changes nothing), so the ray may skip constructing.
    match.shapelessNever = true

    try {
      return bot.world.raycast(headPos, dir.normalize(), range, match) === null
    } catch (_) {
      return false
    }
  }

  // Check visibility to an entity by tracing to a point near its head/upper body.
  function canSeeEntity (entity) {
    if (!entity || !entity.position) return false
    const samplePos = entity.position.offset(0, (entity.height || 1.8) * 0.9, 0)
    return canSeePoint(samplePos)
  }

  // expose helpers for reuse
  bot.canSeePoint = canSeePoint
  bot.canSeeEntity = canSeeEntity
  bot.canSeeCell = canSeeCell
  bot.isSeeThroughBlock = isSeeThroughBlock

  bot._client.on('unload_chunk', (packet) => {
    delColumn(packet.chunkX, packet.chunkZ)
  })

  function updateBlockState (point, stateId) {
    const oldBlock = blockAt(point)
    bot.world.setBlockStateId(point, stateId)

    const newBlock = blockAt(point)
    // sometimes minecraft server sends us block updates before it sends
    // us the column that the block is in. ignore this.
    if (newBlock === null) {
      return
    }
    if (oldBlock.type !== newBlock.type) {
      const pos = point.floored()
      const painting = paintingsByPos[pos]
      if (painting) deletePainting(painting)
    }
  }

  bot._client.on('update_light', (packet) => {
    let column = bot.world.getColumn(packet.chunkX, packet.chunkZ)
    if (!column) {
      column = new Chunk({ minY: bot.game.minY, worldHeight: bot.game.height })
      bot.world.setColumn(packet.chunkX, packet.chunkZ, column)
    }

    if (bot.supportFeature('newLightingDataFormat')) {
      column.loadParsedLight(packet.skyLight, packet.blockLight, packet.skyLightMask, packet.blockLightMask, packet.emptySkyLightMask, packet.emptyBlockLightMask)
    } else {
      column.loadLight(packet.data, packet.skyLightMask, packet.blockLightMask, packet.emptySkyLightMask, packet.emptyBlockLightMask)
    }
  })

  // Chunk batches are used by the server to throttle the chunks per tick for players based on their connection speed.
  let chunkBatchStartTime = 0
  // The Vanilla client uses nano seconds with its weighted average starting at 2000000 converted to milliseconds that is 2
  let weightedAverage = 2
  // This is used for keeping track of the weight of the old average when updating it.
  let oldSampleWeight = 1

  bot._client.on('chunk_batch_start', (packet) => {
    // Get the time the chunk batch is starting.
    chunkBatchStartTime = Date.now()
  })

  bot._client.on('chunk_batch_finished', (packet) => {
    const milliPerChunk = (Date.now() - chunkBatchStartTime) / packet.batchSize
    // Prevents the MilliPerChunk from being hugely different then the average, Vanilla uses 3 as a constant here.
    const clampedMilliPerChunk = Math.min(Math.max(milliPerChunk, weightedAverage / 3.0), weightedAverage * 3.0)
    weightedAverage = ((weightedAverage * oldSampleWeight) + clampedMilliPerChunk) / (oldSampleWeight + 1)
    // 49 is used in Vanilla client to limit it to 50 samples
    oldSampleWeight = Math.min(49, oldSampleWeight + 1)
    bot._client.write('chunk_batch_received', {
      // Vanilla uses 7000000 as a constant here, since we are using milliseconds that is now 7. Not sure why they pick this constant to convert from nano seconds per chunk to chunks per tick.
      chunksPerTick: 7 / weightedAverage
    })
  })
  bot._client.on('map_chunk', (packet) => {
    addColumn({
      x: packet.x,
      z: packet.z,
      bitMap: packet.bitMap,
      heightmaps: packet.heightmaps,
      biomes: packet.biomes,
      skyLightSent: bot.game.dimension === 'overworld',
      groundUp: packet.groundUp,
      data: packet.chunkData,
      trustEdges: packet.trustEdges,
      skyLightMask: packet.skyLightMask,
      blockLightMask: packet.blockLightMask,
      emptySkyLightMask: packet.emptySkyLightMask,
      emptyBlockLightMask: packet.emptyBlockLightMask,
      skyLight: packet.skyLight,
      blockLight: packet.blockLight
    })

    if (typeof packet.blockEntities !== 'undefined') {
      const column = bot.world.getColumn(packet.x, packet.z)
      if (!column) {
        if (!hideErrors) console.warn('Ignoring block entities as chunk failed to load at', packet.x, packet.z)
        return
      }
      for (const blockEntity of packet.blockEntities) {
        if (blockEntity.x !== undefined) { // 1.17+
          column.setBlockEntity(blockEntity, blockEntity.nbtData)
        } else {
          const pos = new Vec3(blockEntity.value.x.value & 0xf, blockEntity.value.y.value, blockEntity.value.z.value & 0xf)
          column.setBlockEntity(pos, blockEntity)
        }
      }
    }
  })

  bot._client.on('map_chunk_bulk', (packet) => {
    let offset = 0
    let meta
    let i
    let size
    for (i = 0; i < packet.meta.length; ++i) {
      meta = packet.meta[i]
      size = (8192 + (packet.skyLightSent ? 2048 : 0)) *
        onesInShort(meta.bitMap) + // block ids
        2048 * onesInShort(meta.bitMap) + // (two bytes per block id)
        256 // biomes
      addColumn({
        x: meta.x,
        z: meta.z,
        bitMap: meta.bitMap,
        heightmaps: packet.heightmaps,
        skyLightSent: packet.skyLightSent,
        groundUp: true,
        data: packet.data.slice(offset, offset + size)
      })
      offset += size
    }

    assert.strictEqual(offset, packet.data.length)
  })

  bot._client.on('multi_block_change', (packet) => {
    // multi block change
    for (let i = 0; i < packet.records.length; ++i) {
      const record = packet.records[i]

      let blockX, blockY, blockZ
      if (bot.supportFeature('usesMultiblockSingleLong')) {
        blockZ = (record >> 4) & 0x0f
        blockX = (record >> 8) & 0x0f
        blockY = record & 0x0f
      } else {
        blockZ = record.horizontalPos & 0x0f
        blockX = (record.horizontalPos >> 4) & 0x0f
        blockY = record.y
      }

      let pt
      if (bot.supportFeature('usesMultiblock3DChunkCoords')) {
        pt = new Vec3(packet.chunkCoordinates.x, packet.chunkCoordinates.y, packet.chunkCoordinates.z)
      } else {
        pt = new Vec3(packet.chunkX, 0, packet.chunkZ)
      }

      pt = pt.scale(16).offset(blockX, blockY, blockZ)

      if (bot.supportFeature('usesMultiblockSingleLong')) {
        updateBlockState(pt, record >> 12)
      } else {
        updateBlockState(pt, record.blockId)
      }
    }
  })

  bot._client.on('block_change', (packet) => {
    const pt = new Vec3(packet.location.x, packet.location.y, packet.location.z)
    updateBlockState(pt, packet.type)
  })

  bot._client.on('explosion', (packet) => {
    // explosion
    const p = new Vec3(packet.x, packet.y, packet.z)
    if (packet.affectedBlockOffsets) {
      // TODO: server no longer sends in 1.21.3. Is client supposed to compute this or is it sent via normal block updates?
      packet.affectedBlockOffsets.forEach((offset) => {
        const pt = p.offset(offset.x, offset.y, offset.z)
        updateBlockState(pt, 0)
      })
    }
  })

  bot._client.on('spawn_entity_painting', (packet) => {
    const pos = new Vec3(packet.location.x, packet.location.y, packet.location.z)
    const painting = new Painting(packet.entityId,
      pos, packet.title, paintingFaceToVec[packet.direction])
    addPainting(painting)
  })

  bot._client.on('entity_destroy', (packet) => {
    // destroy entity
    packet.entityIds.forEach((id) => {
      const painting = paintingsById[id]
      if (painting) deletePainting(painting)
    })
  })

  bot._client.on('update_sign', (packet) => {
    const pos = new Vec3(packet.location.x & 0xf, packet.location.y, packet.location.z & 0xf)

    // TODO: warn if out of loaded world?
    const column = bot.world.getColumn(packet.location.x >> 4, packet.location.z >> 4)
    if (!column) {
      return
    }

    const blockAt = column.getBlock(pos)

    blockAt.signText = [packet.text1, packet.text2, packet.text3, packet.text4].map(text => {
      if (text === 'null' || text === '') return ''
      return JSON.parse(text)
    })
    column.setBlock(pos, blockAt)
  })

  bot._client.on('tile_entity_data', (packet) => {
    if (packet.location !== undefined) {
      const column = bot.world.getColumn(packet.location.x >> 4, packet.location.z >> 4)
      if (!column) return
      const pos = new Vec3(packet.location.x & 0xf, packet.location.y, packet.location.z & 0xf)
      column.setBlockEntity(pos, packet.nbtData)
    } else {
      const tag = packet.nbtData
      const column = bot.world.getColumn(tag.value.x.value >> 4, tag.value.z.value >> 4)
      if (!column) return
      const pos = new Vec3(tag.value.x.value & 0xf, tag.value.y.value, tag.value.z.value & 0xf)
      column.setBlockEntity(pos, tag)
    }
  })

  bot.updateSign = (block, text, back = false) => {
    const lines = text.split('\n')
    if (lines.length > 4) {
      bot.emit('error', new Error('too many lines for sign text'))
      return
    }

    for (let i = 0; i < lines.length; ++i) {
      if (lines[i].length > 45) {
        bot.emit('error', new Error('Signs have a maximum of 45 characters per line'))
        return
      }
    }

    let signData
    if (bot.supportFeature('sendStringifiedSignText')) {
      signData = {
        text1: lines[0] ? JSON.stringify(lines[0]) : '""',
        text2: lines[1] ? JSON.stringify(lines[1]) : '""',
        text3: lines[2] ? JSON.stringify(lines[2]) : '""',
        text4: lines[3] ? JSON.stringify(lines[3]) : '""'
      }
    } else {
      signData = {
        text1: lines[0] ?? '',
        text2: lines[1] ?? '',
        text3: lines[2] ?? '',
        text4: lines[3] ?? ''
      }
    }

    bot._client.write('update_sign', {
      location: block.position,
      isFrontText: !back,
      ...signData
    })
  }

  // if we get a respawn packet and the dimension is changed,
  // unload all chunks from memory.
  let dimension
  let worldName
  function dimensionToFolderName (dimension) {
    if (bot.supportFeature('dimensionIsAnInt')) {
      return dimensionNames[dimension]
    } else if (bot.supportFeature('dimensionIsAString') || bot.supportFeature('dimensionIsAWorld')) {
      return worldName
    }
  }
  // only exposed for testing
  bot._getDimensionName = () => worldName

  async function switchWorld () {
    if (bot.world) {
      if (storageBuilder) {
        await bot.world.async.waitSaving()
      }

      // Notify and detach EVERY per-coordinate listener. The old walk over
      // bot._events only handled the single-listener case (`typeof listener
      // === 'function'`) — with 2+ listeners on one coordinate the entry is
      // an array, so none of them got the (null, null) world-unload notify
      // nor removed, leaving stale per-coordinate listeners on both bot and
      // world across dimension switches.
      for (const name of bot.eventNames()) {
        if (typeof name === 'string' && name.startsWith('blockUpdate:')) {
          bot.emit(name, null, null)
          for (const listener of bot.listeners(name)) {
            bot.off(name, listener)
          }
        }
      }

      for (const [x, z] of Object.keys(bot.world.async.columns).map(key => key.split(',').map(x => parseInt(x, 10)))) {
        bot.world.unloadColumn(x, z)
      }

      if (storageBuilder) {
        bot.world.async.storageProvider = storageBuilder({ version: bot.version, worldName: dimensionToFolderName(dimension) })
      }
    } else {
      bot.world = new World(null, storageBuilder ? storageBuilder({ version: bot.version, worldName: dimensionToFolderName(dimension) }) : null).sync
      if (bot._blockStateRemapper && typeof bot.world.setStateRemapper === 'function') {
        bot.world.setStateRemapper(bot._blockStateRemapper)
      }
      startListenerProxy()
    }
  }

  bot._client.on('login', (packet) => {
    if (bot.supportFeature('dimensionIsAnInt')) {
      dimension = packet.dimension
      worldName = dimensionToFolderName(dimension)
    } else if (bot.supportFeature('spawnRespawnWorldDataField')) { // 1.20.5+
      dimension = packet.worldState.dimension
      worldName = packet.worldState.name
    } else {
      dimension = packet.dimension
      worldName = /^minecraft:.+/.test(packet.worldName) ? packet.worldName : `minecraft:${packet.worldName}`
    }
    switchWorld()
  })

  bot._client.on('respawn', (packet) => {
    if (bot.supportFeature('dimensionIsAnInt')) { // <=1.15.2
      if (dimension === packet.dimension) return
      dimension = packet.dimension
    } else if (bot.supportFeature('spawnRespawnWorldDataField')) { // 1.20.5+
      if (dimension === packet.worldState.dimension) return
      if (worldName === packet.worldState.name && packet.copyMetadata === true) return // don't unload chunks if in same world and metaData is true
      dimension = packet.worldState.dimension
      worldName = packet.worldState.name
    } else { // >= 1.15.2
      if (dimension === packet.dimension) return
      if (worldName === packet.worldName && packet.copyMetadata === true) return // don't unload chunks if in same world and metaData is true
      // Metadata is true when switching dimensions however, then the world name is different
      dimension = packet.dimension
      worldName = packet.worldName
    }
    switchWorld()
  })

  let listener
  let listenerRemove
  function startListenerProxy () {
    if (listener) {
      // custom forwarder for custom events
      bot.off('newListener', listener)
      bot.off('removeListener', listenerRemove)
    }
    // standardized forwarding
    const forwardedEvents = ['blockUpdate', 'chunkColumnLoad', 'chunkColumnUnload']
    for (const event of forwardedEvents) {
      bot.world.on(event, (...args) => bot.emit(event, ...args))
    }
    const blockUpdateRegex = /blockUpdate:\(-?\d+, -?\d+, -?\d+\)/
    listener = (event, listener) => {
      if (blockUpdateRegex.test(event)) {
        bot.world.on(event, listener)
      }
    }
    listenerRemove = (event, listener) => {
      if (blockUpdateRegex.test(event)) {
        bot.world.off(event, listener)
      }
    }
    bot.on('newListener', listener)
    bot.on('removeListener', listenerRemove)
  }

  bot.findBlock = findBlock
  bot.canSeeBlock = canSeeBlock
  bot.blockAt = blockAt
  bot._updateBlockState = updateBlockState
  bot.waitForChunksToLoad = waitForChunksToLoad
}

function onesInShort (n) {
  n = n & 0xffff
  let count = 0
  for (let i = 0; i < 16; ++i) {
    count = ((1 << i) & n) ? count + 1 : count
  }
  return count
}
