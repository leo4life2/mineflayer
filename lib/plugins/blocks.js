...
function inject (bot, { version, storageBuilder, hideErrors }) {
  ...
  function canSeeBlock (block) {
    ...
    const shouldLog = ((block.name || '').toLowerCase() === 'oak_log')
    ...
    const range = dir.norm() + 0.75 // generous epsilon to ensure intersection with target
    ...
    const match = (inputBlock, iter) => {
      ...
      // Always stop if we reached the target block
      if (inputBlock.position.equals(block.position)) return true
      // Skip see-through blocks, continue ray
    ...
    try {
      ...
      if (shouldLog) console.log(`[canSeeBlock] Not visible: raycast error: ${err?.message || String(err)}`)
      return false
    }

    const visible = !!(hitBlock && hitBlock.position && hitBlock.position.equals(block.position))
    if (!visible && shouldLog) {
      if (hitBlock) {
        const blockerName = hitBlock.name || 'unknown'
        const blockerPos = hitBlock.position ? hitBlock.position.toString() : 'unknown'
        console.log(`[canSeeBlock] Not visible: blocked by ${blockerName} at ${blockerPos}`)
      } else {
        const targetName = block.name || 'unknown'
        const shapeInfo = Array.isArray(block.shapes) ? `shapes=${block.shapes.length}` : 'shapes=none'
        const bbox = block.boundingBox
        console.log(`[canSeeBlock] Not visible: ray did not intersect target (${targetName}); ${shapeInfo}, boundingBox=${bbox}`)
      }
    }
    return visible
  }
