...
function inject (bot, options) {
  ...
  bot._client.on('playerChat', (data) => {
    ...
    if (bot.supportFeature('clientsideChatFormatting')) {
      ...
      // Handle both numeric and object types for registryIndex
      let registryIndex
      if (typeof data.type === 'number') {
        registryIndex = data.type
      } else if (data.type && typeof data.type.registryIndex === 'number') {
        registryIndex = data.type.registryIndex
      } else {
        // Default to 0 for standard chat if we can't determine the type
        registryIndex = 0
      }
