  ...
  async function placeEntityWithOptions (referenceBlock, faceVector, options) {
    ...
    if (type === 'boat') {
      if (bot.supportFeature('useItemWithOwnPacket')) {
        // Same rotation law as inventory.js activateItem: populate the
        // protocol-degree look, never {0,0}/absent (server-side rotation snap,
        // rig-proven on Paper 1.21.1 — bow-25694 probes 2026-07-24).
    ...
    return new Promise((resolve, reject) => {
      function listener (entity) {
        ...
        if (entity.name === mobName && dist < maxDistance) {
          // only detach on a match — detaching on any spawn meant one
          // unrelated entity made the wait time out
