...
function inject (bot) {
  ...
  // Mounted entities (passengers) receive no movement packets of their own:
  // the server only moves the vehicle, and clients are expected to derive
  // rider positions from the set_passengers linkage. Without this, a
  // passenger's model position goes stale at its mount point while the
  // vehicle (and the real entity) rides away. Derive passenger positions
  // from the vehicle on every vehicle position update, recursively
  // (entities stack: player on donkey on boat).
  // Y attachment: minecraft-data ships no per-entity mount offsets, so use
  // vanilla's generic Entity#getMountedYOffset (0.75 * vehicle height) as
  // the estimate; x/z always match the vehicle exactly.
  function updatePassengerPositions (vehicle, seen) {
    if (!vehicle.passengers || vehicle.passengers.length === 0) return
    if (!seen) seen = new Set([vehicle.id])
    const yOffset = (vehicle.height ?? 0) * 0.75
    for (const passenger of vehicle.passengers) {
      if (seen.has(passenger.id)) continue // guard against cyclic linkage from bad packets
      seen.add(passenger.id)
      passenger.position.set(vehicle.position.x, vehicle.position.y + yOffset, vehicle.position.z)
      bot.emit('entityMoved', passenger)
      updatePassengerPositions(passenger, seen)
    }
  }
  // Shared with the vehicle_control plugin: a clientbound vehicle_move
  // rubber-band moves the vehicle without any normal entity-move packet, so
  // the rejection handler needs the same passenger sync applied there too.
  bot._updatePassengerPositions = updatePassengerPositions
  ...
  bot._client.on('rel_entity_move', (packet) => {
    ...
    updatePassengerPositions(entity)
  })
  ...
  bot._client.on('damage_event', (packet) => { // 1.20+
    ...
    if (!entity) return // damage for an entity we don't track (despawned or never spawned)
    const source = packet.sourceCauseId > 0 ? (bot.entities[packet.sourceCauseId - 1] ?? null) : null // damage_event : SourceCauseId : The ID + 1 of the entity responsible for the damage, if present. If not present, the value is 0
  ...
  bot._client.on('entity_metadata', (packet) => {
    ...
    if (bot.supportFeature('mcDataHasEntityMetadata')) {
      ...
      // Every entity's metadata flows through this handler, but oxygenLevel is
      // bot self-state: only the bot's own air_supply may drive it (same guard
      // the legacy breath.js path has). Without this, any nearby drowning mob's
      // air ticks overwrite the bot's oxygen reading.
      if (metas.air_supply != null && bot.entity && packet.entityId === bot.entity.id) {
  ...
  // unlink a passenger from its current vehicle (both directions)
  function unlinkPassenger (passenger) {
  ...
  // link a passenger to a vehicle (both directions), moving it off any
  // previous vehicle first
  function linkPassenger (passenger, vehicle) {
    if (passenger.vehicle !== vehicle) unlinkPassenger(passenger)
    passenger.vehicle = vehicle
    if (vehicle && !vehicle.passengers.includes(passenger)) {
      vehicle.passengers.push(passenger)
    }
  }

  // attaching to a vehicle — only 1.8.x stacks (mounts) entities through
  // attach_entity (minecraft-data feature attachStackEntity); from 1.9 the
  // packet is leash-only and set_passengers (setPassengerStackEntity)
  // carries mount linkage, so building vehicle links from it there would
  // corrupt the model (leashed mobs would ride their holder).
  if (bot.supportFeature('attachStackEntity')) {
    bot._client.on('attach_entity', (packet) => {
      const passenger = fetchEntity(packet.entityId)
      const vehicle = packet.vehicleId === -1 ? null : fetchEntity(packet.vehicleId)

      linkPassenger(passenger, vehicle)
  ...
  bot._client.on('set_passengers', ({ entityId, passengers }) => {
    ...
    // set_passengers carries the FULL passenger list for this vehicle:
    // anyone previously linked but absent from the new list has dismounted
    // and must be unlinked, or their position would track the vehicle forever.
    if (vehicle && vehicle.passengers) {
      for (const previous of vehicle.passengers.slice()) {
        if (!passengerEntities.includes(previous)) unlinkPassenger(previous)
      }
    }

    for (const passengerEntity of passengerEntities) {
      linkPassenger(passengerEntity, vehicle)
    }

    if (bot.entity) {
      if (passengers.includes(bot.entity.id)) {
        if (bot.vehicle !== vehicle) {
          bot.vehicle = vehicle
          bot.emit('mount')
        }
      } else if (bot.vehicle && bot.vehicle === vehicle) {
        // the bot was dropped from its vehicle's passenger list
        const originalVehicle = bot.vehicle
        bot.vehicle = null
        bot.emit('dismount', originalVehicle)
      }
  ...
  // dismounting when the vehicle (or a passenger) despawns.
  // NB: entityGone is emitted on bot, not bot._client — listening on the
  // client (as this once did) means the cleanup never ran and despawned
  // vehicles kept ghost passenger links.
  bot.on('entityGone', (entity) => {
    ...
    if (entity.passengers) {
      for (const passenger of entity.passengers.slice()) {
        unlinkPassenger(passenger)
      }
    }
    unlinkPassenger(entity)
  })
  ...
  function useOn (target, clickPosition) {
    // TODO: check if not crouching will make make this action always use the item
    // Truthful-click (capability-fruit F2): the vanilla client right-click
    // sends INTERACT_AT (mouse 2, with the clicked point relative to the
    // entity) followed by INTERACT (mouse 0). Entities that resolve the
    // interaction in interactAt (armor stands equipping the held armor
    // piece being the canonical case) never see a bare INTERACT, which is
    // why the old single-packet path no-op'd on them with a success story.
    // clickPosition is the ABSOLUTE point being clicked (defaults stay
    // exactly at the old single-packet behavior when omitted).
    if (clickPosition) {
      useEntity(target, 2,
        clickPosition.x - target.position.x,
        clickPosition.y - target.position.y,
        clickPosition.z - target.position.z)
    }
    useEntity(target, 0)
  }
  ...
  function moveVehicle (left, forward) {
    if (bot.supportFeature('newPlayerInputPacket')) {
      ...
      // jump flag 0x00, not 0x01: the old constant "jump pressed" bit made
      // every steer packet a jump input. 0x02 (dismount) stays the dismount()
      // channel below. NB: measured on 1.21.1, steer_vehicle moves nothing
      // server-side for a packet client — vehicle travel must be authored via
      // bot.vehicleControl.authorVehicleMove (vehicle_control plugin).
  ...
  function useEntity (target, leftClick, x, y, z) {
    ...
    // x === 0 is a legal click coordinate — the old `x && y && z` guard
    // silently downgraded axis-aligned clicks to a plain interact.
    if (x !== undefined && y !== undefined && z !== undefined) {
      bot._client.write('use_entity', {
        ...
        y,
        z,
        hand: 0, // interact_at carries a hand on modern protocols
        sneaking
      })
    } else {
      bot._client.write('use_entity', {
        target: target.id,
        mouse: leftClick,
        hand: 0, // interact (mouse 0) carries a hand on modern protocols
        sneaking
      })
    }
  }
