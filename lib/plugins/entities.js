...
function inject (bot) {
  ...
  bot._client.on('set_passengers', ({ entityId, passengers }) => {
    ...
    if (passengers.includes(bot.entity.id)) {
      ...
      // 1.21.x removal path: a dismount arrives as set_passengers(vehicleId, [])
      // (or a passenger list that no longer contains the bot). The inclusion
      // branch above can never observe a removal, so without this branch
      // bot.vehicle stayed poisoned after every successful dismount and the
      // 'dismount' event never fired.
      const originalVehicle = bot.vehicle
      bot.vehicle = null
      bot.emit('dismount', originalVehicle)
    }
  ...
  // (bot-level event: entity_destroy emits bot.emit('entityGone', entity);
  // this was registered on bot._client, which never emits it — a killed/
  // despawned vehicle left bot.vehicle poisoned forever.)
  ...
  function mount (target) {
    ...
    // KNOWN BROKEN on 1.21.x: useEntity() omits the required `hand` field, so
    // the use_entity packet never serializes. Use bot.activateEntity(target)
    // (inventory plugin, sends hand: 0) to mount — that is the working path.
    useEntity(target, 0)
  }
  ...
  function dismount () {
    if (bot.vehicle) {
      if (bot.supportFeature('newPlayerInputPacket')) {
        // Vanilla dismount is SNEAK, not jump (jump charges horse-jump).
        bot._client.write('player_input', {
          inputs: {
            sneak: true
          }
