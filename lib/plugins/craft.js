...
function inject (bot) {
  ...
  // (F21 craft_result_desync) the craft batch currently driving clicks; a
  // caller that abandons the await (abort preemption) MUST NOT leave the click
  // chain running — bot.cancelCraft() flags the session and the chain halts at
  // the next click boundary instead of interleaving with the preempting
  // command's clicks over the shared cursor (run-11 line 316-317 trigger).
  ...
  async function craft (recipe, count, craftingTable) {
    ...
    const checkCancelled = () => {
      if (session.cancelled) throw new Error('craft cancelled — click chain halted before the next click')
    }

    try {
      if (!craftingTable) {
        // (F21) Personal-grid craft: never ASSUME the grid is empty.
        // 1) A vanilla player cannot craft in the 2x2 with a container open —
        //    close it properly first (clicking window 0 under an open
        //    container is silently dropped by the server: guaranteed timeout).
        // 2) Reconcile the server-side grid (close_window(0)): debris parked
        //    by any earlier abandoned/corrupted sequence is returned to the
        //    inventory before we place ingredients, so shapeless crafts can
        //    never mismatch against invisible leftovers (run-11's 42
        //    consecutive plank failures) and occupied-slot clicks can never
        //    swallow the cursor stack.
        if (bot.currentWindow) bot.closeWindow(bot.currentWindow)
        await bot.reconcileCraftingGrid()
      }
      for (let i = 0; i < count; i++) {
        checkCancelled()
        await craftOnce(recipe, craftingTable, checkCancelled)
      }

      closeCraftingWindow()
    } catch (err) {
      closeCraftingWindow()
      // (F21) error/cancel path: a half-placed grid or a stranded cursor
      // stack would otherwise stay parked server-side and poison every later
      // shapeless craft. Return it to the inventory the way a vanilla client
      // does (close_window(0)); table windows are already handled by
      // closeCraftingWindow above.
      if (!craftingTable) {
        try { await bot.reconcileCraftingGrid() } catch { /* reconcile is best-effort on the error path */ }
      }
      throw new Error(err)
    } finally {
      if (activeCraftSession === session) activeCraftSession = null
    }
  }

  // (F21) halt the in-flight craft click chain at its next click boundary.
  // For callers whose await was abandoned (withAbort preemption): call this at
  // abort time so the background chain stops mutating the shared cursor/grid;
  // the chain's own error path then reconciles the grid. Restorative phases
  // (returning the cursor stack, grabbing an already-produced result) are NOT
  // interrupted — only new ingredient placements are.
  function cancelCraft () {
    if (activeCraftSession) activeCraftSession.cancelled = true
  }
  ...
  async function craftOnce (recipe, craftingTable, checkCancelled = () => {}) {
    if (craftingTable) {
    ...
    async function startClicking (window, w, h) {
      ...
      // (F21) grid slots THIS chain places ingredients into — the end-of-craft
      // consumption model (updateOutShape) is scoped to exactly these, so it
      // can never stomp a server correction for a slot the chain never touched.
      ...
      async function clickShape () {
        // (F21) cancellation boundary: halt BEFORE the next destructive
        // placement click; restorative phases below are never interrupted.
        checkCancelled()
      ...
      async function updateOutShape () {
        // (F21) client-side consumption model, SCOPED. Taking the verified
        // result consumed one ingredient from each slot this chain placed
        // into — model exactly that. The old blanket null over every grid
        // slot stomped fresher server corrections for slots the chain never
        // touched (wire-caught: true grid-content corrections overwritten
        // ~120ms later), which made real grid desyncs invisible to the
        // client forever. Slots we DID use can still race a correction, but
        // any craft that ends in rejection/timeout now flows into the error
        // path's grid reconcile, which restores server truth wholesale.
        if (!recipe.outShape) {
          for (const _slot of usedGridSlots) {
            window.updateSlot(_slot, null)
          }
          return
        }
        const slotsToClick = []
        for (let y = 0; y < recipe.outShape.length; ++y) {
          ...
          for (let x = 0; x < row.length; ++x) {
            ...
            if (item !== null || usedGridSlots.has(_slot)) window.updateSlot(_slot, item)
          }
        }
