...
function inject (bot, { hideErrors }) {
  ...
  // Personal 2x2 craft-grid reconcile (F21 craft_result_desync). The player's
  // InventoryMenu craft grid (window 0: result slot 0 + grid slots 1-4) lives
  // SERVER-side and persists for the whole session — an abandoned or corrupted
  // click sequence parks ingredients there invisibly (no resync channel ever
  // volunteers those slots), after which every shapeless craft mismatches, the
  // server pins result slot 0 to null, and retries swap whole cursor stacks
  // into the occupied grid (the material black hole; mission run-11's 42
  // consecutive plank failures). A vanilla client sends close_window(0) every
  // time the E-screen closes; sending it here makes the server return grid and
  // cursor contents to the inventory (vanilla InventoryMenu.removed →
  // placeItemBackInInventory), with corrections arriving on the -2/-1 channels
  // handled below — wire-proven heal (lane F21 s7d: RCON-verified item return,
  // next craft passes in 305ms).
  // Local model: after the close, the server's grid and cursor are GUARANTEED
  // empty — mirror exactly that (slots 0-4 + cursor; armor 5-8 untouched),
  // then wait for the returned-item burst to settle so callers count materials
  // on truth, not on the pre-return deficit.
  // Never valid while a container window is open: the server closes the ACTIVE
  // menu regardless of the id in a serverbound close packet, so this would
  // silently close the container server-side and desync it client-side.
  async function reconcileCraftingGrid () {
    if (bot.currentWindow) return false
    bot._client.write('close_window', { windowId: 0 })
    for (let i = 0; i <= 4; i++) {
      if (bot.inventory.slots[i]) bot.inventory.updateSlot(i, null)
    }
    if (bot.inventory.selectedItem) bot.inventory.selectedItem = null
    updateHeldItem()
    await waitForClickSettle(bot.inventory)
    return true
  }

  function copyInventory (window) {
    // (F45 halfopen-wipe) Never copy from a window the server never populated:
    // between the open_window packet (currentWindow set) and its window_items
    // (slots filled) every slot is null — copying that copies 36 nulls over
    // the whole client inventory model. Live trigger: openWindowWithRetry's
    // windowOpen-timeout path force-closes exactly such a half-open window
    // (rig C3: total self-model wipe, picks/planks/cobble all count 0 while
    // the server kept everything).
    if (window.id !== 0 && !window.serverPopulated) return
  ...
  // (F45 inventory_self_model_desync) LIVE shared-segment mirror. While a
  // container is open the server syncs the player's main+hotbar through the
  // CONTAINER's window id, and local click predictions land on the container
  // window too — bot.inventory (the self-model every HUD prompt and craft
  // precondition reads) historically learned of the session's deltas only via
  // copyInventory at CLIENT-initiated close. A SERVER-initiated close
  // (stillValid fail: walked away, block broken — routine under pathfinding
  // load) dropped the whole session: withdrawn items / craft results became
  // invisible (rig C1: stone_pickaxe client 0 vs server 1, spruce_planks
  // client 0 vs server 3 — the owner-watched F45 shape), deposits resurrected
  // client-side (rig C2: cobblestone client 29 vs server 9). Mirroring every
  // shared-segment updateSlot into bot.inventory as it happens closes that
  // door and also makes mid-session HUD reads truthful (rig C5: pickups were
  // client-invisible until close). In-place count mutations that bypass
  // updateSlot (prismarine-windows drop-click decrements) are settled by the
  // guarded copyInventory that now runs on BOTH close paths.
  const cloneItem = (item) => (item ? Object.assign(Object.create(Object.getPrototypeOf(item)), item) : null)
  function mirrorSharedSegment (window) {
    if (!window || window.id === 0) return
    if (typeof window.inventoryStart !== 'number' || typeof window.inventoryEnd !== 'number') return
    window.on('updateSlot', (slot, oldItem, newItem) => {
      if (slot < window.inventoryStart || slot >= window.inventoryEnd) return
      const invSlot = slot - window.inventoryStart + 9
      if (invSlot < 9 || invSlot > 44) return
      if (Item.equal(bot.inventory.slots[invSlot], newItem, true)) return
      // clone: updateSlot stamps .slot onto the object it stores — the
      // container view and the inventory view must not share one mutable item
      bot._setSlot(invSlot, cloneItem(newItem), bot.inventory)
    })
  }

  function prepareWindow (window) {
    clearPendingWindowOpen()
    mirrorSharedSegment(window)
  ...
  bot._client.on('close_window', (packet) => {
    ...
    if (oldWindow && oldWindow.id !== 0) {
      // (F45 walkaway door) settle the session's shared segment into the
      // self-model exactly like a client-initiated close does. The live
      // mirror keeps them coherent event-by-event; this copy catches the
      // in-place count mutations that never emitted updateSlot. Guarded:
      // copyInventory refuses unpopulated (half-open) windows.
      copyInventory(oldWindow)
      stateIds.delete(oldWindow.id)
    }
    bot.currentWindow = null
  ...
  // Per-slot degrade-gracefully guard (capability-fruit F1): a stack whose
  // network payload prismarine-item cannot convert must never abort the rest
  // of a window_items/set_slot sync. It degrades to an id+count item with
  // components flagged unknown instead of throwing out of the handler (which
  // would leave every later slot of the sync dark). Slots the protocol layer
  // already salvaged as prefix stubs arrive pre-flagged via
  // componentsTruncated and keep that flag on the resulting Item.
  let slotDegradeCount = 0
  function safeFromNotch (networkItem) {
    try {
      const item = Item.fromNotch(networkItem)
      if (item && networkItem?.componentsTruncated) item.componentsUnknown = true
      return item
    } catch (err) {
      let stub = null
      try {
        // retry with the component/nbt payload stripped — keeps id+count
        // visible (works on both the components era and the nbt era)
        stub = Item.fromNotch({
          ...networkItem,
          components: [],
          removeComponents: [],
        ...
        if (stub) stub.componentsUnknown = true
      } catch { stub = null }
      if (++slotDegradeCount === 1 || slotDegradeCount % 100 === 0) {
        console.warn(`[inventory] slot payload degraded to ${stub ? 'id+count' : 'empty'} (${slotDegradeCount} so far): ${err.message}`)
      }
      return stub
    }
  }
  bot._client.on('set_slot', (packet) => {
    const wid = normWindowId(packet.windowId)
    const newItem = safeFromNotch(packet.item)
    if (wid === -1) {
    ...
    if (wid === -2) {
      ...
      if (cur && invSlot >= 9 && invSlot <= 44) {
        cur.updateSlot(cur.inventoryStart + (invSlot - 9), newItem ? safeFromNotch(packet.item) : null)
      }
      return
    }
    ...
    if (wid === 0) {
      // (F45 C5 class) a direct w=0 write landing mid-container-session must
      // also reach the open window's shared segment — otherwise the close-time
      // copyInventory stomps it with the stale container view. Mirror image of
      // the -2 branch above; the live mirror's Item.equal guard makes the
      // reflected write idempotent.
      const cur = bot.currentWindow
      if (cur && packet.slot >= 9 && packet.slot <= 44 &&
          typeof cur.inventoryStart === 'number') {
        cur.updateSlot(cur.inventoryStart + (packet.slot - 9), cloneItem(newItem))
      }
    }
  })
  bot._client.on('window_items', (packet) => {
    ...
    if (!window || window.id !== wid) {
      ...
      if (wid > 0 && items.length >= 36) {
        ...
        for (let i = 0; i < 36; ++i) {
          bot.inventory.updateSlot(9 + i, safeFromNotch(items[base + i]))
        }
        updateHeldItem()
      }
      return
    ...
    for (let i = 0; i < packet.items.length; ++i) {
      const item = safeFromNotch(packet.items[i])
      window.updateSlot(i, item)
    }
    // (F45) full state received — this window is now a legal copyInventory
    // source (see the halfopen-wipe guard there).
    window.serverPopulated = true
    ...
    if (packet.carriedItem !== undefined) {
      window.selectedItem = safeFromNotch(packet.carriedItem)
    }
    updateHeldItem()
  ...
  bot.reconcileCraftingGrid = reconcileCraftingGrid
