...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  // Dismount physics-resume guard state (see tickDismountResume below):
  // -1 = no grace window pending; >= 0 = physics ticks left until physics is
  // force-resumed because the server never repositioned us after a dismount.
  const DISMOUNT_RESUME_GRACE_TICKS = 10
  let dismountResumeTicks = -1
  ...
  bot._client.on('position', (packet) => {
    ...
    dismountResumeTicks = -1 // server repositioned us: any dismount grace is settled
    bot.jumpTicks = 0
    lastSentYaw = bot.entity.yaw
    lastSentPitch = bot.entity.pitch
  ...
  // --- Dismount physics-resume guard ----------------------------------------
  // 'mount' gates shouldUsePhysics off and, in stock mineflayer, ONLY a
  // clientbound position packet ever sets it true again. On a normal dismount
  // the server teleports the rider immediately (measured ~1 ms), so the gate
  // reopens before the next physics tick. But when the vehicle is DESTROYED
  // under the bot (mob kills the horse, boat breaks on collision, entity
  // despawn, /kill), vanilla drops the rider via set_passengers/remove with NO
  // position packet: physics stays gated forever. The bot then hangs mid-air
  // sending zero serverbound move packets — everything riding physicsTick
  // (pathfinder, combat, skills) is dead — and the first stray look or
  // position-sync echo latches vanilla's anti-flight watchdog
  // (ServerGamePacketListenerImpl.floating), whose counter increments every
  // server tick with no packet needed: kicked with
  // multiplayer.disconnect.flying after 80 ticks (~4 s).
  // Vanilla-client parity: the notchian client computes a local dismount
  // position and resumes simulation immediately. Guard: after 'dismount', if
  // no server position packet arrives within DISMOUNT_RESUME_GRACE_TICKS,
  // resume physics from the entity's current (vehicle-synced) position at
  // rest; gravity then produces honest descending move packets that clear
  // `floating` long before the watchdog fires.
  //
  // shouldUsePhysics writer inventory (this guard is the only writer added):
  //   login    -> false   (grace cleared: pre-spawn, server will position us)
  //   position -> true    (grace cleared: the normal-path race winner)
  //   mount    -> false   (grace cleared: re-mounted during grace stays gated)
  //   respawn  -> false   (grace cleared: respawn teleport is coming)
  //   grace expiry -> true (this guard; A2 dismount-without-teleport only)
  function tickDismountResume () {
    if (dismountResumeTicks < 0) return
    if (shouldUsePhysics) { // a server position packet won the race: normal path
      dismountResumeTicks = -1
      return
    }
    dismountResumeTicks--
    if (dismountResumeTicks >= 0) return
    // Grace expired with physics still gated: the server never repositioned
    // us after the dismount. Resume simulation from where the entity is.
    recoverEntityState('dismountResume')
    bot.entity.velocity.set(0, 0, 0)
    bot.entity.onGround = false
    bot.jumpTicks = 0
    shouldUsePhysics = true
    const p = bot.entity.position
    trace('dismount_physics_resume', { x: p.x, y: p.y, z: p.z })
    console.warn(`[physics] event=dismount_physics_resume x=${p.x.toFixed(2)} y=${p.y.toFixed(2)} z=${p.z.toFixed(2)} — no server teleport after dismount, resuming simulation`)
    bot.emit('physicsResumedAfterDismount')
  }

  bot.on('dismount', () => {
    if (!shouldUsePhysics) dismountResumeTicks = DISMOUNT_RESUME_GRACE_TICKS
  })
