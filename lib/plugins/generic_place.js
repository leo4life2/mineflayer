...
// 1.19 (protocol 759) introduced the serverbound block-action `sequence`
// field and the matching acknowledge (block_changed_ack) response.
const SEQUENCED_BLOCK_ACTIONS_PROTOCOL = 759

function inject (bot) {
  const Item = require('prismarine-item')(bot.registry)

  // Per-bot serverbound block-action sequence counter (1.19+). The server
  // acknowledges the highest sequence it has fully processed each tick
  // (acknowledge_player_digging carrying only sequenceId); real sequence
  // numbers are what make that ack usable as a per-click server response —
  // with a constant 0 the ack says nothing about WHICH click was processed.
  let blockActionSequence = 0
  bot._nextBlockActionSequence = () => ++blockActionSequence
  bot._supportsBlockActionAck = () =>
    (bot.registry?.version?.version ?? 0) >= SEQUENCED_BLOCK_ACTIONS_PROTOCOL
  /**
   *
  ...
  async function _genericPlace (referenceBlock, faceVector, options) {
    ...
    if (bot.supportFeature('blockPlaceHasHeldItem')) {
      ...
      // Real sequence on 1.19+ so the server's block_changed_ack names this
      // click; the allocated number rides back on the CALLER'S options object
      // (per-call, so concurrent placements never race a shared field).
      const sequence = bot._supportsBlockActionAck() ? bot._nextBlockActionSequence() : 0
      if (options) options._sequence = sequence
      bot._client.write('block_place', {
        location: pos,
        direction: vectorToDirection(faceVector),
        hand: handToPlaceWith,
